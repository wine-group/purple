    ...
    def update(self, timestamp: datetime, tx_rate: float, radio_capacity: float,
        ...
        try:
            ...
            if current_time - self.last_update_time >= self.update_interval:
                ...
                if self.useful_capacity is None:
                    self.useful_capacity = tx_capacity_val * 0.80  # Apply safety factor immediately. Default of 0.85 (last 15% is always bad)
                else:
...
class RadioMonitor:
    ...
    def __init__(self, 
        ...
        self.capacity_estimator = EnhancedESCEstimator()
    ...
    #     """Update traffic control rate on local interface"""
    #     try:
    ...
    #             # Build tc command
    #             cmd = [
    #                 'sudo', 'tc', 'qdisc', 'replace', 'dev', self.local_interface,
    #                 'root', 'cake', f'rate {rate_bps}bit'
    #             ]
    ...
    #             if result.stderr:
    #                 self.logger.warning(f"TC command warning: {result.stderr}")
                
    #             self.logger.info(f"Updated TC rate to {rate_mbps:.2f} Mbps")
    #             self.last_tc_update = current_time
                
    #     except subprocess.CalledProcessError as e:
    #         self.logger.error(f"Failed to update TC rate: {e.stderr}")
    #     except Exception as e:
    #         self.logger.error(f"Error updating TC rate: {str(e)}")


    # Adding margins... to try stop cake oscillation
    # def _update_tc_rate(self, rate_mbps: float) -> None:
    #     """Update traffic control rate via SSH"""
    #     try:
    #         current_time = time.time()
    ...
    #             # Setup SSH connection for tc updates
    ...
    #                 # Build tc command
    #                 # For existing
    #                 # sudo tc qdisc change dev eth0 handle 8001: cake bandwidth 100mbit
    #                 cmd = f"sudo tc qdisc change dev {self.local_interface} handle 8001: cake bandwidth {rate_bps}bit"

    #                 # To replace:
    #                 # cmd = f"sudo tc qdisc replace dev {self.local_interface} root cake bandwidth {rate_bps}bit"
