...
class SplitPing:
    ...
    def calculate_jitter(self):
        ...
        # Sort measurements by sequence number
        sorted_measurements = sorted(self.measurements, key=lambda x: x['sequence'])
        
        # Calculate packet-to-packet delay variations
        forward_variations = []
        reverse_variations = []
        total_variations = []
        ...
        for i in range(1, len(sorted_measurements)):
            # Forward jitter
            prev_forward = sorted_measurements[i-1]['forward_delay']
            curr_forward = sorted_measurements[i]['forward_delay']
            forward_variations.append(abs(curr_forward - prev_forward))
            
            # Reverse jitter
            prev_reverse = sorted_measurements[i-1]['reverse_delay']
            curr_reverse = sorted_measurements[i]['reverse_delay']
            reverse_variations.append(abs(curr_reverse - prev_reverse))
            
            # Total jitter
            prev_total = sorted_measurements[i-1]['total_delay']
            curr_total = sorted_measurements[i]['total_delay']
            total_variations.append(abs(curr_total - prev_total))
        
        return {
            'forward_jitter': {
                'mean': statistics.mean(forward_variations),
                'median': statistics.median(forward_variations),
                'max': max(forward_variations),
                'std': statistics.stdev(forward_variations)
            },
            'reverse_jitter': {
                'mean': statistics.mean(reverse_variations),
                'median': statistics.median(reverse_variations),
                'max': max(reverse_variations),
                'std': statistics.stdev(reverse_variations)
            },
            'total_jitter': {
                'mean': statistics.mean(total_variations),
                'median': statistics.median(total_variations),
                'max': max(total_variations),
                'std': statistics.stdev(total_variations)
            }
        }
        ...
        try:
            ...
            if len(data) == struct.calcsize('!Qd'):  # Initial packet
                ...
                measurement = {
                    ...
                    'forward_timestamp': server_time,
                    'reverse_timestamp': recv_time
                }
    ...
    def save_measurements(self):
        ...
        # Calculate jitter statistics
        jitter_stats = self.calculate_jitter()
